...
function sitemapToggle($,trigger,element){
	var $trigger = $(trigger);
	var $elem = $(element);
	...
	$trigger.prepend('<span id="sitemapToogleImg"></span>');

	$trigger.click(function(){
		$elem.slideToggle('slow', function(){
			// animation complete
			$trigger.find("#sitemapToogleImg").toggleClass("up");
		});
	});

}

/* djob 15.05.2013 fuer mailto auf Fehlerseiten (404, 503) */
...
jQuery(document).ready(function($) {
	init($);
	sitemapToggle($,"#smToggle","#sitemap");

	$("#mainNav select").styleSelect({
